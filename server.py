...
def predict():
    if request.form['model'] == 'Support Vector Machines':
        result = classification[SVM_Model(float(request.form['sepal_length']), float(request.form['sepal_width']),
                                          float(request.form['petal_length']), float(request.form['petal_width']))]

    if request.form['model'] == 'Decision Tree':
        result = classification[DecisionTree(float(request.form['sepal_length']), float(request.form['sepal_width']),
                                             float(request.form['petal_length']), float(request.form['petal_width']))]

    if request.form['model'] == 'Neural Networks':
        result = classification[NeuralNetwork(float(request.form['sepal_length']), float(request.form['sepal_width']),
                                              float(request.form['petal_length']), float(request.form['petal_width']))]
